...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long inst_all_cnt     = ULONG_MAX;
      ...
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.inst_all_cnt     < inst_all_cnt    )  inst_all_cnt     = res.inst_all_cnt     ;
      ...
      printk("All instructions fetching times: %u\n", (unsigned)inst_all_cnt);
