...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long mem_load_time    = ULONG_MAX;
      unsigned long mem_store_time   = ULONG_MAX;
      unsigned long inst_num         = ULONG_MAX;
      unsigned long mem_load_cycle   = ULONG_MAX;
      unsigned long mem_store_cycle  = ULONG_MAX;
      unsigned long inst_fetch_cycle = ULONG_MAX;
      unsigned long jump_num         = ULONG_MAX;
      int succ = 1;
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.mem_load_time    < mem_load_time   )  mem_load_time    = res.mem_load_time    ;
        if (res.mem_store_time   < mem_store_time  )  mem_store_time   = res.mem_store_time   ;
        if (res.inst_num         < inst_num        )  inst_num         = res.inst_num         ;
        if (res.mem_load_cycle   < mem_load_cycle  )  mem_load_cycle   = res.mem_load_cycle   ;
        if (res.mem_store_cycle  < mem_store_cycle )  mem_store_cycle  = res.mem_store_cycle  ;
        if (res.inst_fetch_cycle < inst_fetch_cycle)  inst_fetch_cycle = res.inst_fetch_cycle ;
        if (res.jump_num         < jump_num        )  jump_num         = res.jump_num         ;
      }
      ...
      printk("Memory load counts: %u\n", (unsigned)mem_load_time);
      printk("Memory store counts: %u\n", (unsigned)mem_store_time);
      printk("Instruction numbers: %u\n", (unsigned)inst_num);
      printk("Memory load cycles: %u\n", (unsigned)mem_load_cycle);
      printk("Memory store cycles: %u\n", (unsigned)mem_store_cycle);
      printk("Instruction fetching cycles: %u\n", (unsigned)inst_fetch_cycle);
      printk("Total happened PC jump: %u\n", (unsigned)jump_num);

      printk("Memory reference time: %u\n", (unsigned)(mem_load_time + mem_store_time));
      // printk("CPI (Cycles-per-instruction): %u\n", (unsigned)(msec / inst_num));
    }
  }
